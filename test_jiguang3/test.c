...
static void project_jiguang3_tempature_map(int16_t *input, float *output);
#endif
        ...
        while (xlsxioread_sheet_next_row(sheet)) {
            ...
            if(s_excel_row >= 1) //filter the first line(header)
            {
                ...
                while (xlsxioread_sheet_next_cell_float(sheet, &cell_value)) {
                    ...
                    else if(excel_col >= 76 && excel_col < 80){                 // cell voltage
                        sox_excel_input[s_excel_row-1].vol[excel_col-76] = (int)cell_value;
                    }
                    else if(excel_col >= 108 && excel_col < 111){               // cell temperature
...
#elif CFG_SOX_PORT_SIM_PROJECT == 4
    project_jiguang3_tempature_map(input0.tmp, sox_excel_input[0].tmp);
#endif
...
#elif CFG_SOX_PORT_SIM_PROJECT == 4
        project_jiguang3_tempature_map(input.tmp, sox_excel_input[i].tmp);
#endif
        // caculate
        sox_task(&input);
