...
#elif CFG_SOX_PORT_SIM_PROJECT == 4
static void project_yitiji_cell_tempature_map(int16_t *input, float *output);
#endif
        ...
        while (xlsxioread_sheet_next_row(sheet)) {
            ...
            if(s_excel_row >= 1) //filter the first line(header)
            {
                ...
                while (xlsxioread_sheet_next_cell_float(sheet, &cell_value)) {
                    if(excel_col == 3){                                         // group voltage
                        sox_excel_input[s_excel_row-1].grpVol = cell_value;
                    }
                    else if(excel_col == 4){                                   // group current    
                        sox_excel_input[s_excel_row-1].cur = cell_value;
                    }   
                    else if(excel_col == 5){                                   // group soc(microcontroller calculated)
                        sox_excel_input[s_excel_row-1].groupSoc = cell_value;
                    }
                    else if(excel_col >= 13 && excel_col < 21){                 // cell voltage
                        sox_excel_input[s_excel_row-1].vol[excel_col-13] = (int)(cell_value*1000);
                    }
                    else if(excel_col >= 21 && excel_col < 30){               // cell temperature
                        sox_excel_input[s_excel_row-1].tmp[excel_col-21] = cell_value;
                    }
                    excel_col++;
                }
            }
...
#elif CFG_SOX_PORT_SIM_PROJECT == 4
    project_yitiji_cell_tempature_map(input0.tmp, sox_excel_input[0].tmp);
#endif
...
#elif CFG_SOX_PORT_SIM_PROJECT == 4
    project_yitiji_cell_tempature_map(input.tmp, sox_excel_input[i].tmp);
#endif
        // caculate
        sox_task(&input);
...
static void project_yitiji_cell_tempature_map(int16_t *input, float *output)
{
    ...
    {
        input[j] = (roundf(output[j-1]*10) + roundf(output[j+1]*10))/2 ;
    }
    input[7] = roundf(output[8]*10);
}
#endif